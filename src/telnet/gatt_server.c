...
// #define GATT_SERVICE_DEVICE_NAME     "KVM"
...
static be_telnet_gatt_msg_handler_t gatt_msg_handler_cb = NULL;
...
static uint8_t adv_service_uuid128[32] = {
        ...
        0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x0C, 0x0B, 0x00, 0x00,
};
...
static void gatt_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            // gl_profile.service_id.id.uuid.uuid.uuid16 = GATT_SERVICE_UUID;
            // dn(gl_profile.service_id.id.uuid.uuid.uuid16)

            esp_err_t set_dev_name_ret = esp_ble_gap_set_device_name(gatt_service_name);
            if (set_dev_name_ret) {
...
#endif
            esp_ble_gatts_create_service(gatts_if, &gl_profile.service_id, GATT_NUM_HANDLE);
            break;
        ...
        case ESP_GATTS_WRITE_EVT: {
            // ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, conn_id %d, trans_id %" PRIu32 ", handle %d", param->write.conn_id,
                ...
                if (gl_profile.descr_handle == param->write.handle && param->write.len == 2) {
                    ...
                    if (descr_value == 0x0001) {
                        ...
                        if (gatt_msg_handler_cb) {
                            gatt_msg_handler_cb(param->write.value, param->write.len);
                        }
                    }
                } else if (gatt_msg_handler_cb) {
                    gatt_msg_handler_cb(param->write.value, param->write.len);
                }
            }
        ...
        case ESP_GATTS_CREATE_EVT:
            ...
            // gl_profile.char_uuid.uuid.uuid16 = GATT_CHAR_UUID;
            // dn(gl_profile.char_uuid.uuid.uuid16)
...
void be_telnet_gatt_server_start(const char * name, uint16_t characteristicID, uint16_t serviceID, uint16_t appID) {
    ...
    gl_profile.char_uuid.len = ESP_UUID_LEN_16;
    gl_profile.char_uuid.uuid.uuid16 = characteristicID;

    strncpy(gatt_service_name, name, sizeof(gatt_service_name) - 1) ;

    esp_err_t ret = esp_ble_gatts_register_callback(gatts_event_handler);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "esp_ble_gatts_register_callback(), error code = %x", ret);
        return;
    }
    ret = esp_ble_gap_register_callback(gap_event_handler);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "esp_ble_gap_register_callback(), error code = %x", ret);
        return;
    }
...
void be_telnet_gatt_server_stop() {
    if (!running) {
        ESP_LOGI(GATTS_TAG, "GATT service is not  running");
        return;
    }
    esp_err_t ret;
    ret = esp_ble_gatts_app_unregister(gl_profile.gatts_if);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "gatts app unregister error, error code = %x", ret);
    } else {
        ESP_LOGI(GATTS_TAG, "gatts app unregistered");
    }
    ret = esp_ble_gatts_stop_service(gl_profile.service_handle);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "stop service failed, error code = %x", ret);
    } else {
        ESP_LOGI(GATTS_TAG, "service stopped");
    }
    ret = esp_ble_gatts_delete_service(gl_profile.service_handle);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "delete service failed, error code = %x", ret);
    } else {
