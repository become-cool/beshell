...
#define NVS_INT_JS_GETTER(name,ctype,jstype)                   \
    JSValue NVS::jsRead##name(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {    \
        CHECK_ARGC(1)                           \
...
#define NVS_INT_JS_SETTER(name,ctype,jstype,apitype)               \
    JSValue NVS::jsWrite##name(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {    \
        CHECK_ARGC(2)                                                   \
        ARGV_TO_CSTRING(0,key)                                           \
        ctype value = 0 ;                                               \
...
namespace be {
    ...
    JSValue NVS::jsErase(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
        CHECK_ARGC(1)
        ARGV_TO_CSTRING(0,key)
